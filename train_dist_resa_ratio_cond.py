...
def main(args):
    ...
    exp_path = Path(args.output_dir) / (args.id + '-' + args.ood) / '-'.join([args.arch, args.clf_type, args.training, args.dist+'_cond', 'r_'+str(args.id_ratio)])
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if args.dist == 'maha':
            ...
            _, weights_id = get_cond_maha_weight(train_loader_id_test, clf, num_classes, cat_mean, precision)
        elif args.dist == 'negative_logit':
            _, weights_candidate_ood = get_cond_negative_logit_weight(train_candidate_loader_ood_test, clf)
            _, weights_id = get_cond_negative_logit_weight(train_loader_id_test, clf)
        else:
