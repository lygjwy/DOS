...
for w in prob logit; do
    for t in fix var; do
        for r in True False; do
            # tuning
            python train_cla_resa.py --id cifar100 --ood tiny_images --training $t --weight_type $w --replacement $r --pretrain ./outputs/cifar100/wrn40/cla_best.pth --lr 0.001 --epochs 10 --gpu_idx 0
            # retraining
            python train_cla_resa.py --id cifar100 --ood tiny_images --training $t --weight_type $w --replacement $r --lr 0.1 --epochs 100 --gpu_idx 0
        done
    done
done