...
def main(args):
    ...
    exp_path = Path(args.output_dir) / (args.id + '-' + args.ood) / '-'.join([args.arch, args.clf_type, args.training, args.dist, 'r_'+str(args.id_ratio)])
    ...
    if args.training == 'uni':
        clf = get_clf(args.arch, num_classes, clf_type=args.clf_type)
    elif args.training == 'abs':
        clf = get_clf(args.arch, num_classes+1, clf_type=args.clf_type)
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if args.dist == 'maha':
            ...
            weights_id, _ = get_cond_maha_weight(train_loader_id_test, clf, num_classes, cat_mean, precision)
        elif args.dist == 'negative_logit':
            weights_candidate_ood, _ = get_cond_negative_logit_weight(train_candidate_loader_ood_test, clf)
            weights_id, _ = get_cond_negative_logit_weight(train_loader_id_test, clf)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id_ratio', type=float, default=0.9)
