...
def get_abs_kl(data_loader, clf):
    clf.eval()
    ...
    for sample in data_loader:
        ...
        softmax = torch.softmax(logit[:, :-1], dim=1)
        uniform_dist = torch.ones_like(softmax) * (1 / softmax.shape[1])
        kl_score.extend(torch.sum(F.kl_div(softmax.log(), uniform_dist, reduction='none'), dim=1).tolist())
    
    return np.asarray(abs_weight), np.asarray(kl_score)

# def get_inter_idxs_sampled(data_loader, clf, total_num, target_num, abs_ratio, kl_ratio):
#     abs_ood, kl_ood = get_abs_kl(data_loader, clf)

#     abs_sorted_idxs = np.argsort(abs_ood)[:int(total_num * abs_ratio)]
#     kl_sorted_idxs = np.argsort(kl_ood)[:int(total_num * kl_ratio)]
#     inter_idxs = [idx for idx in abs_sorted_idxs if idx in kl_sorted_idxs]
...
#         print('AR:', abs_ratio, 'KR:', kl_ratio)
#         idxs_sampled = get_inter_idxs_sampled(data_loader, clf, total_num, target_num, abs_ratio, kl_ratio)
#     else:
#         idxs_sampled = random.sample(inter_idxs, k=target_num)
    
#     return idxs_sampled

def get_inter_idxs_sampled(data_loader, clf, total_num, target_num, abs_ratio, kl_value):
    abs_ood, kl_ood = get_abs_kl(data_loader, clf)

    abs_sorted_idxs = set(np.argsort(abs_ood)[:int(total_num * abs_ratio)])
    kl_sorted_idxs = np.argsort(kl_ood)
    
    # get the dividing point
    sorted_kl_ood = kl_ood[kl_sorted_idxs]
    kl_ood_spt = bisect.bisect(sorted_kl_ood, kl_value)
    print(kl_ood_spt / len(kl_sorted_idxs))
    kl_target_idxs = set(kl_sorted_idxs[:kl_ood_spt])

    # inter_idxs = [idx for idx in abs_sorted_idxs if idx in kl_target_idxs]
    inter_idxs = set(abs_sorted_idxs).intersection(kl_target_idxs)
    print('Inter length:', len(inter_idxs))
    ...
    if len(inter_idxs) < target_num:
        idxs_sampled = random.sample(abs_sorted_idxs, k=target_num)
    else:
        idxs_sampled = random.sample(inter_idxs, k=target_num)
...
def main(args):
    ...
    # if args.replacement:
    #     exp_path = Path(args.output_dir) / (args.id + '-' + args.ood) / '-'.join([args.arch, args.training, args.scheduler, 'trip-wr', 'b_'+str(args.beta), 'ar_'+str(args.abs_ratio), 'kr_'+str(args.kl_ratio)])
    # else:
    #     exp_path = Path(args.output_dir) / (args.id + '-' + args.ood) / '-'.join([args.arch, args.training, args.scheduler, 'trip-wor', 'b_'+str(args.beta), 'ar_'+str(args.abs_ratio), 'kr_'+str(args.kl_ratio)])
    if args.diff:
        exp_path = Path(args.output_dir) / (args.id + '-' + args.ood) / '-'.join([args.arch, args.training, args.scheduler, 'trip', 'b_'+str(args.beta), 'wd', 'ar_'+str(args.abs_ratio)])
    else:
        exp_path = Path(args.output_dir) / (args.id + '-' + args.ood) / '-'.join([args.arch, args.training, args.scheduler, 'trip', 'b_'+str(args.beta), 'wod', 'ar_'+str(args.abs_ratio)])
    ...
    # all_indices_ood = set(range(len(train_all_set_ood)))
    # print(len(all_indices_ood))
    indices_sampled_ood = []
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        # remove the previously sampled ood idxs
        # if args.replacement:
            # all_indices_ood = all_indices_ood
        # else:
            # all_indices_ood = list(filter(lambda indice: indice not in indices_sampled_ood, all_indices_ood))
            # all_indices_ood = all_indices_ood.difference(set(indices_sampled_ood))
            # print(len(all_indices_ood))
        
        # indices_candidate_ood = random.sample(all_indices_ood, k=args.candidate_ood_size)
        ...
        # kl_value = np.sort(get_kl_score(test_loader_id, clf))[int(len(test_set_id) * args.kl_ratio)-1]
        if args.diff:
            kl_value = np.mean(get_kl_score(test_loader_id, clf))
            print(kl_value)
        else:
            kl_value = float('inf')

        idxs_sampled = get_inter_idxs_sampled(test_candidate_loader_ood, clf, args.candidate_ood_size, args.sampled_ood_size_factor * len(train_set_id), args.abs_ratio, kl_value)
        ...
        # weights_ood_test = np.asarray(get_abs_weight(test_loader_ood, clf))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', help='dir to store experiment artifacts', default='es-a')
    ...
    parser.add_argument('--abs_ratio', type=float, default=0.5)
